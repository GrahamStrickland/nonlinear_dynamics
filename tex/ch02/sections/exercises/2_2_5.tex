% Exercise 2.2.5
In Figure~\ref{fig2_2_5vecfield}, we have a plot of the vector field
and various graphs
...
\end{equation}
Note that there are no fixed points for
Equation~\eqref{eq2_2_5vecfield}, so that these
are not included in Figure~\ref{fig2_2_5vecfield}.
\begin{figure}[!ht]
  \includegraphics[scale=0.8, center]{../plots/ch02/fig2_2_5vecfield.pdf}
...
\begin{equation*}
  \begin{split}
    ...
    \Leftrightarrow x
    &= 2\tan^{-1}\biggl[\sqrt{3}\tan{\biggl(\frac{\sqrt{3}}{4}t +
    c_3\biggr)} \biggr],
  \end{split}
\end{equation*}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
so that $c_3 = 0$, and we have a solution corresponding to the
initial condition $x(0) = 0$
given by
\[
  x(t) =
  2\tan^{-1}\biggl[\sqrt{3}\tan{\biggl(\frac{\sqrt{3}}{4}t\biggr)} \biggr].
\]

In Figure~\ref{fig2_2_5graph}, we have a plot of this solution.
\begin{figure}[!ht]
  \includegraphics[scale=0.8, center]{../plots/ch02/fig2_2_5graph.pdf}
  \caption{Graph of a solution of $\dot{x} = 1 +
  \frac{1}{2}\cos{x}$\label{fig2_2_5graph}}
\end{figure}
